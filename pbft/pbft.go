...
func (p *PBFT) HandleRequest(data []byte) {
	...
	switch command(cmd) {
	case cRequest:
		tbegin = time.Now() // 主节点共识开始时间戳
		p.handleClientRequest(content)
	case cPrePrepare:
		tbegin = time.Now() //非主节点共识开始时间戳
		p.handlePrePrepare(content)
	case cPrepare:
		//tbegin = time.Now() // 收到 prepare的开始时间
...
func (p *PBFT) handlePrepare(content []byte) {
	...
	if _, ok := p.messagePool[pre.Digest]; !ok {
		...
		if count >= specifiedCount && !p.isCommitBordcast[pre.Digest] {
			...
			// tend = time.Now()
			// fmt.Println("prepare 阶段耗时", tend.Sub(tbegin))
			//进行提交信息的广播
...
func (p *PBFT) handleCommit(content []byte) {
	...
	if _, ok := p.prePareConfirmCount[c.Digest]; !ok {
		...
		p.lock.Lock()
		f := (nodeCount - 1) / 3 // 每个节点至少收到 N-f -1 个commit（不包括自己）
		if count >= 2*f && !p.isReply[c.Digest] && p.isCommitBordcast[c.Digest] {
			...
			fmt.Println(info)
			tend = time.Now()
			fmt.Println(p.node.nodeID, "共识耗时：", tend.Sub(tbegin))
			fmt.Println("正在reply客户端 ...")
			replyClient([]byte(info), p.messagePool[c.Digest].ClientAddr)
