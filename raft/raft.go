...
func (rf *Raft) handlevote(content []byte) {
	...
	if !recv.Voteflag {
		...
		if rf.currentLeader != "-1" || rf.state != FOLLOWER || rf.votedFor != "-1" {
			...
			tcpDial(msg, recv.Node.Addr)
			fmt.Printf("投票成功，已投%s节点,term %d\n", recv.Node.ID, rf.currentTerm)
		}
	} else {
		rf.voteCh <- true
	}
...
func (rf *Raft) handleLmsg(content []byte) {
	...
	if !recv.ConfirmFlag {
		...
		tcpDial(msg, recv.Node.Addr) // 回复leader 收到了
		fmt.Printf("本节点已收到leader日志:%s 已回复，待确认\n", recv.Content)
	} else {
		rf.ConfirmCh <- true
	}
}
...
func (rf *Raft) handleCmsg(content []byte) {
	fmt.Println("收到客户端请求")
	if rf.currentLeader == "-1" { // 没领导者，返回
		fmt.Println("No Leader!")
		return
	}

	if rf.currentLeader != rf.me { // 领导者不是自己，转发到领导者
		r := jointMessage(Cmessage, content)
		fmt.Println("当前节点不是leader，正在重定向...")
		tcpDial(r, nodeTable[rf.currentLeader])
		return
	}
	// 主节点共识开始
	tbegin = time.Now()
	...
	for {
		...
		case ok := <-rf.ConfirmCh:
			...
			if num >= (raftCount-1)/2 {
				fmt.Printf("全网已超过半数节点接收到消息：%s\nraft验证通过，准备同步至follower...", r.Content) // 主节点到这共识就完成了
				rf.lastMessageTime = millisecond()
				rf.broadcast(commit, content)
				//replyclient
				tcpDial([]byte(r.Content), "192.168.242.129:8888")
				tend = time.Now()
				fmt.Println(rf.me, "节点共识耗时", tend.Sub(tbegin))
				//stopper.Stop()
				return
			}
		}
...
func (rf *Raft) handleCommit(content []byte) {
	...
	rf.lastMessageTime = millisecond()
	tend = time.Now()
	fmt.Println(rf.me, "节点共识耗时", tend.Sub(tbegin)) // follower节点
	//stopper.Stop()
}
