...
func genRsaKeys(node string) {
	...
	//for i := 0; i <= 4; i++ {   strconv.Itoa(i)
	if !isExist("./Keys/" + node) {
		err := os.Mkdir("./Keys/"+node, 0644)
		if err != nil {
			log.Panic()
		}
	} else {
	...
	priv, pub := getKeyPair()
	privFileName := "Keys/" + node + "/" + node + "_RSA_PIV"
	...
	pubFileName := "Keys/" + node + "/" + node + "_RSA_PUB"
	...
	file2.Write(pub)
	//}
	fmt.Println("已为节点", node, "生成RSA公私钥")
}
